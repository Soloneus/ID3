...
train_data_m = pd.read_csv("C:/Users/kajet/Desktop/test.csv", sep=";",
...
def calc_total_entropy(train_data):  # get total entrophy
    ...
    total_entr = 0
    class_list = get_conclusions(train_data)  # onclusions list
    for c in class_list:  # for each conclusion in the dataset
        total_class_count = sum(train_data["Książka"][c] == 1)  # number of conclusion
        ...
        for h2 in train_data[h1].columns:
            ...
            for numb in [0, 1]:
                ...
                for c in class_list:
                    ...
                    s2 = (train_data["Książka"][c] == 1)  # rows with conclusion c
...
while (len(table_and_node) > 0):  # split tables untill found purity
    ...
    for c in table_and_node:  # split table and check for purity
        ...
        if check_purity(splitted_0) == 0:  # if table is not pure add another condition to node structure
            name = str(row) + chr(alph) + ". " + max_gain(splitted_0)[0] + ": " + max_gain(splitted_0)[1]
            node = Node(name, parent=c[1], weight=" nie", shape="rectangle", color="orange")
            temp.append([splitted_0, node])
        else:  # if table is pure add conclusion to node structure
            name = str(row) + chr(alph) + ". " + check_purity(splitted_0)[0] + ": " + check_purity(splitted_0)[1]
            node = Node(name, parent=c[1], weight=" nie", color="darkgreen")
        alph += 1
        classes = get_parent_classes(c[1])
        splitted_1 = del_cls(splitted[1], classes)
        if check_purity(splitted_1) == 0:
            name = str(row) + chr(alph) + ". " + max_gain(splitted_1)[0] + ": " + max_gain(splitted_1)[1]
            node = Node(name, parent=c[1], weight=" tak", shape="rectangle", color="orange")
            temp.append([splitted_1, node])
        else:
            name = str(row) + chr(alph) + ". " + check_purity(splitted_1)[0] + ":" + check_purity(splitted_1)[1]
...
DotExporter(root, nodeattrfunc=set_color_shape,
            edgeattrfunc=lambda parent, child: "style=bold,label=%s" % (child.weight or "?")).to_picture("ID3.png")  # export graph to ID3.png

im = Image.open(r"ID3.png")  # open image
im.show()  # show image
